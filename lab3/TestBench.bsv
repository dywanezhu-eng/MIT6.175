...
module mkTestBench#(Fft fft)();
    ...
    rule stream(init);
        ...
        if ( rc != rf ) begin
            ...
            // $display ("Stream %0d failed!", stream_count);
            $finish;
        end
    endrule
    ...
    rule pass (stream_count == 128 && init);
        $display("PASSED");
        $display("It takes %d cycles" , cycle_count);
        $finish;
    endrule
    ...
    rule timeout(init);
        ...
        // $display("feed: %0d, cycle: %0d , stream: %0d", feed_count, cycle_count, stream_count);
